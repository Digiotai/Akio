...
def genAIPrompt(request):
    if request.method == "POST":
        ...
        # Filter or reduce data size for better performance
        # Example: If you need only specific columns or a sample of the data:
        # df = df[['col1', 'col2']].sample(1000)  # Adjust as necessary
        ...
        prompt_eng = (
            f"You are a graphbot that generates Python code using Matplotlib. "
            f"The user has provided data from a database table '{tablename}', which has been stored in a Pandas DataFrame named {df}. "
            f"The DataFrame {df} has the following columns: {metadata_str}. "
            f"Based on the user's request: '{query}', generate Python code that uses the {df} DataFrame to create the appropriate plot using Matplotlib and saves the plot as 'graph.png'. "
            f"Ensure that the code directly references the {df} DataFrame and does not generate or use any other data. "
            f"Only reply with the complete Python code, without any explanations or additional text."
