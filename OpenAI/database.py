...
if __name__ == '__main__':
    ...
    # print(pdd.get_tables_info())
    #print(pdd.delete_table_data("admin@gmail.com","retail_sales_data_60"))
    #print(pdd.delete_main_table())
...
class HanaDBManager:
    ...
    def connect_to_hana(self, user, password, address, port=443):
        try:
            ...
            print("Connected to HANA database successfully.")
            return self.fetch_table_overview()
        except Exception as e:
            print(f"Error connecting to HANA: {e}")
            self.connection = None
            return "Connection failed"

    def ensure_hana_connection(self):
        """
        Ensure the connection to HANA is active. Reconnect if necessary.
        """
        try:
            if self.connection is None:
                print("Reconnecting to HANA database...")
                ...
                try:
                    ...
                    print(f"Connection check failed, reconnecting... Error: {e}")
                    self.connect_to_hana(
                        user=HANA_USER,
                        password=HANA_PASSWORD,
                        address=HANA_ADDRESS,
            ...
            print(f"Error ensuring HANA connection: {e}")
            raise

    def check_if_table_exists(self, table_name):
        """
        Check if a table exists in the HANA database.
        """
        try:
            ...
            with self.connection.cursor() as cursor:
                query = """
                    SELECT COUNT(*) 
                    FROM SYS.TABLES 
                    WHERE TABLE_NAME = ? AND SCHEMA_NAME = 'DBADMIN'
                """
    ...
    def create_user_data_table(self):
        """
        Create the USER_DATA_TABLE only if it does not exist.
        """
        try:
            self.ensure_hana_connection()
            table_exists = self.check_if_table_exists('USER_DATA_TABLE')
            if table_exists:
                print("Table 'USER_DATA_TABLE' already exists.")
                return

            with self.connection.cursor() as cursor:
                query = """CREATE TABLE USER_DATA_TABLE (
                            ...
                            STORED_DATA BLOB,
                            CONSTRAINT UNIQUE_USER_DATA UNIQUE (USER_EMAIL, DATA_NAME)
                            )"""
                cursor.execute(query)
                print("Table 'USER_DATA_TABLE' created successfully.")
        except Exception as err:
            print(f"Error creating table: {err}")
            return str(err)

    def save_or_update_record(self, email, data, file_name):
        """
        Save or update user data records based on the combination of email and file name.
        """
        try:
            self.ensure_hana_connection()
            file_name_clean = file_name.split('.')[0]  # Strip extension
            blob_data = pickle.dumps(data)  # Serialize the data

            with self.connection.cursor() as cursor:
                # Check if a record with the same email and file name exists
                cursor.execute("SELECT RECORD_ID FROM USER_DATA_TABLE WHERE USER_EMAIL = ? AND DATA_NAME = ?",
                               (email, file_name_clean))
                existing_record = cursor.fetchone()

                if existing_record:
                    cursor.execute("""UPDATE USER_DATA_TABLE
                                      SET UPDATED_AT = ?, STORED_DATA = ?
                                      WHERE USER_EMAIL = ? AND DATA_NAME = ?""",
                                   (datetime.now(), blob_data, email, file_name_clean))
                    print(f"Record with email '{email}' and file name '{file_name_clean}' updated successfully.")
                    return "Record updated successfully"
                else:
                    cursor.execute("""INSERT INTO USER_DATA_TABLE (USER_EMAIL, DATA_NAME, UPDATED_AT, CREATED_AT, STORED_DATA)
                                      VALUES (?, ?, ?, ?, ?)""",
                                   (email, file_name_clean, datetime.now(), datetime.now(), blob_data))
                    print(f"New record with email '{email}' and file name '{file_name_clean}' inserted successfully.")
                    return "Record inserted successfully"
        except Exception as err:
            print(f"Error saving or updating record: {err}")
            return str(err)

    def fetch_all_records(self):
        """
        Fetch all records from the USER_DATA_TABLE.
        """
        try:
            self.ensure_hana_connection()
            with self.connection.cursor() as cursor:
            ...
            columns = ['RECORD_ID', 'USER_EMAIL', 'DATA_NAME', 'UPDATED_AT', 'CREATED_AT', 'STORED_DATA']
            df = pd.DataFrame(rows, columns=columns)
            return df
        except Exception as err:
    ...
    def fetch_table_overview(self):
        """
        Fetch an overview of the table without the binary data.
        """
        try:
            df = self.fetch_all_records()
            return df.iloc[:, :-1].to_json() if not df.empty else {}
        except Exception as err:
    ...
    def drop_user_data_table(self):
        """
        Drop the USER_DATA_TABLE.
        """
        try:
            self.ensure_hana_connection()
            with self.connection.cursor() as cursor:
                cursor.execute("DROP TABLE USER_DATA_TABLE CASCADE")
                print("Table 'USER_DATA_TABLE' dropped successfully.")
...
if __name__ == '__main__':
    ...
    # print(hana_db.save_or_update_record('test_user@gmail.com', df1, "TestFile"))
    #
    # # Fetch table info
